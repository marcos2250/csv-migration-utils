...
public class DataUtil {
    ...
    private static final LocalTime ULTIMO_INSTANTE_DIA = new LocalTime(23, 59, 59, 999);
    ...
    public static LocalDate toLocalDate(String pattern, String dataString) {
        return LocalDate.parse(dataString.trim(), getDateTimeFormatter(pattern));
    }
    ...
    public static LocalDateTime toLocalDateTime(String dataHoraString) {
        return getDateTimeFormatter(DATE_TIME_PATTERN).parseDateTime(dataHoraString).toLocalDateTime();
    }
    ...
    public static String toStringNumerica(LocalDateTime dataHora) {
        return toString(dataHora, getDateTimeFormatter(DATE_TIME_PATTERN_NUMERICO));
    }
    ...
    public static String toStringFull(LocalDateTime dataHora) {
        return toString(dataHora, DateTimeFormat.fullDateTime().withZone(ZONE).withLocale(LOCALE));
    }

    private static String toString(BaseLocal baseLocal, DateTimeFormatter formatter) {
    ...
    public static boolean possuiIntersecao(LocalDate dataInicioPesquisa, LocalDate dataFimPesquisa,
            LocalDate dataInicioBase, LocalDate dataFimBase) {
        return getIntervalo(dataInicioPesquisa, dataFimPesquisa).overlaps(getIntervalo(dataInicioBase, dataFimBase));
    }

    public static Interval getIntervalo(LocalDate inicio, LocalDate fim) {
        return new Interval(inicio.toDateTimeAtStartOfDay(), fim.toDateTime(ULTIMO_INSTANTE_DIA));
    }
    ...
    public static boolean primeiraDataPosteriorASegundaData(LocalDate primeiraData, LocalDate segundaData) {
    ...
    public static boolean temMesmoDiaDoMes(LocalDate data1, LocalDate data2, int diaDoMes) {
        return temMesmoDiaDoMes(data1, data2) && (data1.getDayOfMonth() == diaDoMes);
    }

    public static String toMesAnoComMesTextoEAnoAbreviados(LocalDate dataHora) {
        return toString(dataHora, getDateTimeFormatter(DATE_PATTERN_MES_TEXTO_ANO));
    }

    public static String toMesAnoComMesTextoEAno(LocalDate dataHora) {
        return toString(dataHora, getDateTimeFormatter(DATE_PATTERN_MES_TEXTO_COMPLETO_ANO));
    }
